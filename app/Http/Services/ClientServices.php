...
class ClientServices
{
    ...
    {
        $clientInfo = User::where('role', USER_ROLE_CLIENT)
        ...
        return datatables($orderHistory)
            ...
            ->addColumn('order_id', function ($orderHistory) {
                if(auth()->user()->role == USER_ROLE_CLIENT || auth()->user()->role == USER_ROLE_REVIEWER){
                    $orderViewRoute = route('user.orders.task-board.index', $orderHistory->id);
                    $data = '<a href="'. $orderViewRoute .'"><div class="d-flex align-items-center g-5">
                                   <p class="text-nowrap">' . $orderHistory->order_id . '</p>
                                </div></a>';
                }else{
                    $data = '<a href="'. route('admin.client-orders.task-board.index', $orderHistory->id) .'"><div class="d-flex align-items-center g-5">
                                   <p class="text-nowrap">' . $orderHistory->order_id . '</p>
                                </div></a>';
                }
                return $data;
            ...
            ->rawColumns(['status', 'service_name', 'working_status', 'payment_status','order_id'])
            ->make(true);
    }
