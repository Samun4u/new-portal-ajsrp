...
class CouponService
{
    ...
    {
        $data = Coupon::where('tenant_id', auth()->user()->tenant_id)->orderByDesc('id');
    ...
    {
        return Coupon::where('tenant_id', auth()->user()->tenant_id)->find(decrypt($id));
    }
    ...
    public function store($request)
    {
        ...
        try {
            $id = $request->get('id', '');
            if ($id) {
                $coupon = Coupon::where('tenant_id', auth()->user()->tenant_id)->findOrFail(decrypt($id));
            } else {
                $coupon = new Coupon();
            }
    ...
    {
        try {
            $data = Coupon::where('tenant_id', auth()->user()->tenant_id)->findOrFail(decrypt($id));
            $data->delete();
