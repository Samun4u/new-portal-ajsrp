...
class PagesServices
{
    ...
    public function getPageListData($request)
    {
        $pageList = Page::orderByDesc('id');

        return datatables($pageList)
            ->addIndexColumn()
            ->addColumn('sl_no', function ($pageList) {
                return $pageList->id;
            })
            ->addColumn('title', function ($pageList) {
                return $pageList->title;
            })
            ->addColumn('slug', function ($pageList) {
                return $pageList->slug;
            })
            ->addColumn('role_id', function ($pageList) {
                return $pageList->role_id === USER_ROLE_CLIENT ? 'Clients' : 'Reviewers';
            })
            ->addColumn('status', function ($pageList) {
                return $pageList->status === 1 ? 'Active' : 'Inactive';
            })
            ->addColumn('action', function ($data) {
                return "<div class='dropdown dropdown-one'>
                        ...
                        <li>
                        <a class='d-flex align-items-center cg-8' href='" . route('admin.pages.view', $data->slug) . "'>
                        <div class='d-flex'>
                       <svg width='15' height='12' viewBox='0 0 15 12' fill='none' xmlns='http://www.w3.org/2000/svg'><path d='M7.5 8C8.60457 8 9.5 7.10457 9.5 6C9.5 4.89543 8.60457 4 7.5 4C6.39543 4 5.5 4.89543 5.5 6C5.5 7.10457 6.39543 8 7.5 8Z' fill='#5D697A' /><path d='M14.9698 5.83C14.3817 4.30882 13.3608 2.99331 12.0332 2.04604C10.7056 1.09878 9.12953 0.561286 7.49979 0.5C5.87005 0.561286 4.29398 1.09878 2.96639 2.04604C1.6388 2.99331 0.617868 4.30882 0.0297873 5.83C-0.00992909 5.93985 -0.00992909 6.06015 0.0297873 6.17C0.617868 7.69118 1.6388 9.00669 2.96639 9.95396C4.29398 10.9012 5.87005 11.4387 7.49979 11.5C9.12953 11.4387 10.7056 10.9012 12.0332 9.95396C13.3608 9.00669 14.3817 7.69118 14.9698 6.17C15.0095 6.06015 15.0095 5.93985 14.9698 5.83ZM7.49979 9.25C6.857 9.25 6.22864 9.05939 5.69418 8.70228C5.15972 8.34516 4.74316 7.83758 4.49718 7.24372C4.25119 6.64986 4.18683 5.99639 4.31224 5.36596C4.43764 4.73552 4.74717 4.15642 5.20169 3.7019C5.65621 3.24738 6.23531 2.93785 6.86574 2.81245C7.49618 2.68705 8.14965 2.75141 8.74351 2.99739C9.33737 3.24338 9.84495 3.65994 10.2021 4.1944C10.5592 4.72886 10.7498 5.35721 10.7498 6C10.7485 6.86155 10.4056 7.68743 9.79642 8.29664C9.18722 8.90584 8.36133 9.24868 7.49979 9.25Z' fill='#5D697A'/></svg> </div>
                        ...
                        <li>
                            <a class='d-flex align-items-center cg-8' href='" . route('admin.pages.edit', encrypt($data->id)) . "'>
                            <div class='d-flex'>
                        ...
                        <li>
                            <button class='d-flex align-items-center cg-8 border-0 bg-transparent' onclick='deleteItem(\"" . route('admin.pages.delete', encrypt($data->id)) . "\", \"clientListDatatable\")'>
                            <div class='d-flex'>
    ...
    public function getPageListUserData($request)
    {
        $pageList = Page::query()->where('role_id', auth()->user()->role)->where('status', 1)->orderByDesc('id');
        ...
        return datatables($pageList)
            ...
            ->addColumn('title', function ($pageList) {
                //click title and go to view page
                $linkableTitle = "<button class='d-flex align-items-center cg-8 border-0 p-0 bg-transparent' onclick='window.location.href=\"" . route('user.pages.view', $pageList->slug) . "\"'>" . $pageList->title . "</button>";
                return $linkableTitle;
                // return '<a href="' . route('user.pages.view', $pageList->slug) . '">' . $pageList->title . '</a>';
            ...
            ->addColumn('action', function ($data) {
                ...
                return "<ul style='display: flex; flex-direction: column; align-items: flex-end; list-style: none; padding: 0;'>
                    <li>
                        <a class='d-flex align-items-center cg-8' href='" . route('user.pages.view', $data->slug) . "'>
                        <div class='d-flex'>
                       <svg width='15' height='12' viewBox='0 0 15 12' fill='none' xmlns='http://www.w3.org/2000/svg'><path d='M7.5 8C8.60457 8 9.5 7.10457 9.5 6C9.5 4.89543 8.60457 4 7.5 4C6.39543 4 5.5 4.89543 5.5 6C5.5 7.10457 6.39543 8 7.5 8Z' fill='#5D697A' /><path d='M14.9698 5.83C14.3817 4.30882 13.3608 2.99331 12.0332 2.04604C10.7056 1.09878 9.12953 0.561286 7.49979 0.5C5.87005 0.561286 4.29398 1.09878 2.96639 2.04604C1.6388 2.99331 0.617868 4.30882 0.0297873 5.83C-0.00992909 5.93985 -0.00992909 6.06015 0.0297873 6.17C0.617868 7.69118 1.6388 9.00669 2.96639 9.95396C4.29398 10.9012 5.87005 11.4387 7.49979 11.5C9.12953 11.4387 10.7056 10.9012 12.0332 9.95396C13.3608 9.00669 14.3817 7.69118 14.9698 6.17C15.0095 6.06015 15.0095 5.93985 14.9698 5.83ZM7.49979 9.25C6.857 9.25 6.22864 9.05939 5.69418 8.70228C5.15972 8.34516 4.74316 7.83758 4.49718 7.24372C4.25119 6.64986 4.18683 5.99639 4.31224 5.36596C4.43764 4.73552 4.74717 4.15642 5.20169 3.7019C5.65621 3.24738 6.23531 2.93785 6.86574 2.81245C7.49618 2.68705 8.14965 2.75141 8.74351 2.99739C9.33737 3.24338 9.84495 3.65994 10.2021 4.1944C10.5592 4.72886 10.7498 5.35721 10.7498 6C10.7485 6.86155 10.4056 7.68743 9.79642 8.29664C9.18722 8.90584 8.36133 9.24868 7.49979 9.25Z' fill='#5D697A'/></svg> </div>
                            <p class='fs-14 fw-500 lh-17 text-para-text'>" . __('View') . "</p></a>
                        </li>
            ...
            ->rawColumns(['title','role_id','status','action'])
            ->make(true);
    }
    ...
    public function store($request)
    {
        ...
        $request->validate([
            'title' => $request->id ? 'required|unique:pages,title,' . $request->id : 'required|unique:pages,title',
            'content' => 'required',
