...
class ClientBookServices
{
    use ResponseTrait;

    public function getClientBookListData($request, $id = null)
    {

        $clientInfo = BookSubmission::orderByDesc('book_submissions.id');
        if ($id) {
            $clientInfo->with('client')->where('client_id', $id);
            $status = 'all';
        }
...
//            })
            ->addColumn('title', function ($clientInfo) {
                return $clientInfo->title;
            })
            ->addColumn('author', function ($clientInfo) {
                return $clientInfo->author;
            })
            ->addColumn('email', function ($clientInfo) {
                return $clientInfo->email;
            })
            ->addColumn('client_name', function ($clientInfo) {
                $data = '<a href="'. route('admin.client.details', encrypt($clientInfo->client?->id)) .'">
                                <div class="d-flex align-items-center g-5">
                                <p class="text-nowrap">' . $clientInfo->client?->name . '</p>
                                </div></a>';
                return $data;
            })
            ->addColumn('publication_year', function ($clientInfo) {
                return $clientInfo->publication_year;
            })
            ->addColumn('allow_public', function ($clientInfo) {
                return $clientInfo->allow_public ? __("Yes") : __("No");
            })
            ->addColumn('status', function ($clientInfo) {
                if ($clientInfo->status == SUBMISSION_BOOK_STATUS_PUBLISHED) {
                    return '<p class="zBadge zBadge-paid">' . __("Paid") . '</p>';
                } elseif ($clientInfo->status == SUBMISSION_BOOK_STATUS_PENDING) {
                    return '<p class="zBadge zBadge-pending">' . __("Pending") . '</p>';
                } else {
                    return '<p class="zBadge zBadge-cancel">' . __("Cancelled") . '</p>';
                }
            })
            ->addColumn('action', function ($clientInfo) {
                ...
                //view details button ============
                if (
                    auth()->user()->role == USER_ROLE_ADMIN || 
                    auth()->user()->role == USER_ROLE_TEAM_MEMBER ||
                    auth()->user()->role == USER_ROLE_CLIENT
                ) {

                    $route = auth()->user()->role == USER_ROLE_CLIENT ? route('user.submit-your-book.show',$clientInfo->id) : route('admin.submitted-books.show', $clientInfo->id);


                    $return .= "<li><button class='d-flex align-items-center cg-8 border-0 p-0 bg-transparent' onclick='getEditModal(\"" . $route . "\", \"#invoicePreviewModal\")'>";
                    $return .= "<div class='d-flex'>
                            ...
                            <p class='fs-14 fw-500 lh-17 text-para-text'>" . __('View') . "</p></button></li>";
                } else {
            ...
            ->rawColumns(['title','client_name','author','email','publication_year','allow_public','status','action'])
            ->make(true);
    }
    ...
    public function bookCount($clientId=null)
    {
