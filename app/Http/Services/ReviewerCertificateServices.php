...
class ReviewerCertificateServices
{
    use ResponseTrait;

    public function getReviewerCertificateListData(Request $request)
    {

        $reviewerCertificateInfo = ReviewerCertificate::with('reviewer')->where('status', STATUS_ACTIVE);

        if(auth()->user()->role == USER_ROLE_PUBLISHER){
            $reviewerCertificateInfo->whereHas('client_order.client_order_submission', function ($query) {
        ...
        $reviewerCertificateInfo->orderByDesc('id');


        return datatables($reviewerCertificateInfo)
            ->addIndexColumn()
            ->addColumn('sl_no', function ($reviewerCertificateInfo) {
                return $reviewerCertificateInfo->id;
            })
            ->addColumn('title', function ($reviewerCertificateInfo) {
                return $reviewerCertificateInfo->title;
            })
            ->addColumn('client_order_id', function ($reviewerCertificateInfo) {
                return $reviewerCertificateInfo->client_order_id;
            })
            ->addColumn('reviewer_id', function ($reviewerCertificateInfo) {
                return $reviewerCertificateInfo->reviewer->name;
            })
            ->addColumn('affiliations', function ($reviewerCertificateInfo) {
                return $reviewerCertificateInfo->affiliations;
            })
            ->addColumn('paper_title', function ($reviewerCertificateInfo) {
                return $reviewerCertificateInfo->paper_title;
            })
            ->addColumn('journal_name', function ($reviewerCertificateInfo) {
                return $reviewerCertificateInfo->journal_name;
            })
            ->addColumn('action', function ($data) {
                ...
                $actionHtml ="<div class='dropdown dropdown-one'>
                        ...
                        target='_blank' href='" . route('admin.certificate.reviewer.print', encrypt($data->id)) . "'>
                        <div class='d-flex'>
                       <svg width='15' height='12' viewBox='0 0 15 12' fill='none' xmlns='http://www.w3.org/2000/svg'><path d='M7.5 8C8.60457 8 9.5 7.10457 9.5 6C9.5 4.89543 8.60457 4 7.5 4C6.39543 4 5.5 4.89543 5.5 6C5.5 7.10457 6.39543 8 7.5 8Z' fill='#5D697A' /><path d='M14.9698 5.83C14.3817 4.30882 13.3608 2.99331 12.0332 2.04604C10.7056 1.09878 9.12953 0.561286 7.49979 0.5C5.87005 0.561286 4.29398 1.09878 2.96639 2.04604C1.6388 2.99331 0.617868 4.30882 0.0297873 5.83C-0.00992909 5.93985 -0.00992909 6.06015 0.0297873 6.17C0.617868 7.69118 1.6388 9.00669 2.96639 9.95396C4.29398 10.9012 5.87005 11.4387 7.49979 11.5C9.12953 11.4387 10.7056 10.9012 12.0332 9.95396C13.3608 9.00669 14.3817 7.69118 14.9698 6.17C15.0095 6.06015 15.0095 5.93985 14.9698 5.83ZM7.49979 9.25C6.857 9.25 6.22864 9.05939 5.69418 8.70228C5.15972 8.34516 4.74316 7.83758 4.49718 7.24372C4.25119 6.64986 4.18683 5.99639 4.31224 5.36596C4.43764 4.73552 4.74717 4.15642 5.20169 3.7019C5.65621 3.24738 6.23531 2.93785 6.86574 2.81245C7.49618 2.68705 8.14965 2.75141 8.74351 2.99739C9.33737 3.24338 9.84495 3.65994 10.2021 4.1944C10.5592 4.72886 10.7498 5.35721 10.7498 6C10.7485 6.86155 10.4056 7.68743 9.79642 8.29664C9.18722 8.90584 8.36133 9.24868 7.49979 9.25Z' fill='#5D697A'/></svg> </div>
                        ...
                        $actionHtml .= "<li><a class='d-flex align-items-center cg-8' href='" . route('admin.certificate.reviewer.edit', encrypt($data->id)) . "'>
                        ...
                        $actionHtml .= "<li><button class='d-flex align-items-center cg-8 border-0 p-0 bg-transparent' onclick='sendItem(\"" . route('admin.certificate.reviewer.send', encrypt($data->id)) . "\", \"clientListDatatable\")'>
            ...
            ->rawColumns(['reviewer_id','action'])
            ->make(true);
    }

    public function storeReviewerCertificate(Request $request)
    {
        DB::beginTransaction();
        try {

            if ($request->id) {
                $data = ReviewerCertificate::find($request->id);
                $msg = getMessage(UPDATED_SUCCESSFULLY);

                $existReviewerCertificate  = ReviewerCertificate::where('client_order_id', $request->client_order_id)->where('reviewer_id', $request->reviewer_id)->whereNot('id',$data->id)->first();
                if($existReviewerCertificate){
                ...
                $data = new ReviewerCertificate();
                $msg = getMessage(CREATED_SUCCESSFULLY);

                $existReviewerCertificate  = ReviewerCertificate::where('client_order_id', $request->client_order_id)->where('reviewer_id', $request->reviewer_id)->first();
                if($existReviewerCertificate){
            ...
            $hasSubmittedNotes = SubmissionReviewerNotes::where('order_id', $request->client_order_id)
                ->where('created_by', $request->reviewer_id)
                ->exists();

            if (!$hasSubmittedNotes) {
                // Also check formal peer review workflow
                $hasSubmittedNotes = \App\Models\Reviews::where('client_order_id', $request->client_order_id)
            ...
            // if (!$hasSubmittedNotes) {
            //     return $this->error([], __('Reviewer must have submitted reviewer notes or a completed review before a certificate can be issued.'));
            // }

            $data->client_order_id = $request->client_order_id;
            $data->title = $request->title;
            $data->reviewer_id = $request->reviewer_id;
            $data->affiliations = $request->affiliations;
            $data->paper_title = $request->paper_title;
            $data->journal_name = $request->journal_name;
            $data->language = $request->language ?? 'en';
            $data->chief_editor_name = $request->chief_editor_name;
            $data->chief_editor_name_ar = $request->chief_editor_name_ar;

            if ($request->hasFile('signature_image')) {
                $fileManager = new FileManager();
                $uploaded = $fileManager->upload('certificates', $request->signature_image);
                if ($uploaded) {
                    $data->signature_image = $uploaded->id;
                }
            }

            if ($request->hasFile('logo_image')) {
                $fileManager = new FileManager();
                $uploaded = $fileManager->upload('certificates', $request->logo_image);
                if ($uploaded) {
                    $data->logo_image = $uploaded->id;
                }
            }
