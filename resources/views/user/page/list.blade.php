   ...
   @if(count($pageList) > 0)
        ...
        <div
            ...
            {{-- <a href="{{route('admin.pages.add')}}"
               class="border-0 bg-main-color py-8 px-26 bd-ra-8 fs-15 fw-600 lh-25 text-white">+ {{__('Add Page')}}</a> --}}
        </div>
                <!--  -->
        <div class="bg-white bd-one bd-c-stroke bd-ra-10 p-sm-30 p-15">
            <table class="table zTable zTable-last-item-right" id="clientListDatatable">
                ...
                <tr>
                    ...
                    {{-- <th>
                        <div class="text-nowrap">{{__('Slug')}}</div>
                    </th>
                     <th>
                        <div class="text-nowrap">{{__('Who Can See')}}</div>
                    </th>
                    <th>
        ...
        @else
            ...
            <div class="p-sm-30 p-15 bg-white bd-one bd-c-stroke bd-ra-10">
                <div class="create-wrap">
                    ...
                    <h4 class="pb-22 fs-24 fw-500 lh-30 text-title-black text-center">{{__("There is no page available here!")}}</h4>
                    {{-- <a href="{{route('admin.pages.add')}}"
                       class="d-inline-flex bd-ra-8 bg-main-color py-10 px-26 fs-15 fw-600 lh-25 text-white">+{{__("Add Page")}}</a> --}}
                </div>
            </div>
        </div>
   @endif
</span>
    <input type="hidden" id="client-list-route" value="{{route('user.pages.list')}}">
@endsection
@push('script')
    <script src="{{ asset('user/custom/js/pages.js') }}"></script>
@endpush
