...
<span id="searchresult">
   @if(count($reviewerList) > 0)
    <div data-aos="fade-up" data-aos-duration="1000" class="p-sm-30 p-15">
        ...
        <div
            ...
            <a href="{{route('admin.reviewer.add-list')}}"
               class="border-0 bg-main-color py-8 px-26 bd-ra-8 fs-15 fw-600 lh-25 text-white">+ {{__('Add Reviewer')}}</a>
            @endif
        </div>
                <!--  -->
        ...
        @else
            ...
            <div class="p-sm-30 p-15 bg-white bd-one bd-c-stroke bd-ra-10">
                <div class="create-wrap">
                    ...
                    <h4 class="pb-22 fs-24 fw-500 lh-30 text-title-black text-center">{{__("There is no Reviewer available here!")}}</h4>
                    @if(auth()->user()->role !== USER_ROLE_PEER_REVIEWER_MANAGER)
                    <a href="{{route('admin.reviewer.add-list')}}"
                       class="d-inline-flex bd-ra-8 bg-main-color py-10 px-26 fs-15 fw-600 lh-25 text-white">+{{__("Add Reviewer")}}</a>
                    @endif
                </div>
            </div>
        </div>
   @endif
    ...
    <input type="hidden" id="client-list-route" value="{{ route('admin.reviewer.list', request()->query()) }}">
@endsection
@push('script')
