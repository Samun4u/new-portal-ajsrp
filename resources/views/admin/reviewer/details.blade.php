...
@section('content')
                ...
                <div class="col-xl-4 col-md-5">
                    <div class="bd-one bd-c-stroke bd-ra-8 bg-white p-sm-25 p-15">
                        <div class="w-105 h-105 rounded-circle overflow-hidden"><img
                                src="{{asset(getFileUrl($reviewerDetails->image))}}" alt=""/></div>
                            ...
                            <ul class="zList-pb-16">
                                <li class="row flex-wrap">
                                    <div class="col-6"><h4 class="fs-12 fw-500 lh-19 text-title-black">{{__('Name')}}
                                            :</h4></div>
                                    <div class="col-6"><p
                                            class="fs-12 fw-500 lh-19 text-para-text">{{$reviewerDetails->name ?? 'N/A'}}</p>
                                    </div>
                                </li>
                                <li class="row flex-wrap">
                                    <div class="col-6"><h4
                                    ...
                                    <div class="col-6"><p
                                            class="fs-12 fw-500 lh-19 text-para-text">{{$reviewerDetails->email ?? 'N/A'}}</p>
                                    </div>
                                </li>
                                <li class="row flex-wrap">
                                    <div class="col-6"><h4
                                    ...
                                    <div class="col-6">
                                        @if($reviewerDetails && $reviewerDetails->mobile)
                                            <p class="fs-12 fw-500 lh-19 text-para-text">{{$reviewerDetails->mobile}}</p>
                                        @else
                                ...
                                <li class="row flex-wrap">
                                    ...
                                    <div class="col-6">
                                        @if($reviewerDetails && $reviewerDetails->status == STATUS_ACTIVE)
                                ...
                                <li class="row flex-wrap">
                                    <div class="col-6"><h4
                                            class="fs-12 fw-500 lh-19 text-title-black">{{__('Account Status')}} :</h4>
                                    </div>
                                    <div class="col-6 d-flex fs-12 fw-500 lh-19 gap-2">{{ accountStatus($reviewerDetails->status) }}
            ...
            <div class="modal-content bd-c-stroke bd-one bd-ra-10">
                <div class="modal-body p-sm-25 p-15">
                    ...
                    <div
                        ...
                        <h4 class="fs-18 fw-600 lh-22 text-title-black">{{__('Change Account Status')}}</h4>
                        <button type="button"
                    ...
                    <form method="POST" class="ajax reset"
                          action="{{route('admin.reviewer.update-status',['id' => encrypt($reviewerDetails->id)])}}"
                          data-handler="commonResponseWithPageLoad">
                        @csrf
                        <div class="pb-25">
                            <label for="status">{{__('Select Status')}}</label>
                            <select class="form-control" id="status" name="status">
                                <option value="1" {{ $reviewerDetails->status == 1 ? 'selected' : '' }}>{{ __('Active') }}</option>
                                <option value="3" {{ $reviewerDetails->status == 3 ? 'selected' : '' }}>{{ __('Inactive') }}</option>
                                <option value="5" {{ $reviewerDetails->status == 5 ? 'selected' : '' }}>{{ __('Suspended') }}</option>
                            </select>
                        </div>
                        <!-- Button -->
                        <div class="d-flex g-12">
                            <button type="submit"
                                    class="bd-one bd-c-main-color bd-ra-8 py-10 px-26 fs-15 fw-600 lh-25 text-white bg-main-color d-flex justify-content-center">{{__("Submit")}}</button>
                        </div>
                    </form>
                </div>
            </div>
    ...
    <!-- Modal HTML -->
    <input type="hidden" id="client-order-history-route" value="{{route('admin.reviewer.details',['id' => $reviewerDetails->id])}}">
    {{-- <input type="hidden" id="client-invoice-history-route" value="{{route('admin.reviewer.invoice',['id' => $reviewerDetails->id])}}"> --}}
    {{-- <input type="hidden" id="client-activity-log-history-route" value="{{route('admin.reviewer.activity-log-history',['id' => $reviewerDetails->id])}}"> --}}
@endsection
@push('script')
    <script src="{{ asset('admin/custom/js/reviewer.js') }}"></script>
    <script>
