        ...
        <div class="p-sm-30 p-15">
            <div class="max-w-894 m-auto">
                ...
                <div class="d-flex justify-content-between align-items-center g-10 pb-12">
                    <!--  -->
                    <h4 class="fs-18 fw-600 lh-20 text-title-black">{{__("Add Category")}}</h4>
                    <!--  -->
                </div>
                <form class="ajax reset" action="{{route('admin.journals.category.store')}}" method="POST"
                      enctype="multipart/form-data" data-handler="commonResponseRedirect" data-redirect-url="{{route('admin.journals.category.list')}}">
                    @csrf
                            ...
                            <div class="row rg-20 pb-20">
                                <div class="col-12">
                                    ...
                                    <input type="text" class="form-control zForm-control" id="createCategoryName"
                                           placeholder="{{__('Category Name (English)')}}" name="name"/>
                                </div>
                                <div class="col-12">
                                    <label for="createCategoryNameInAr" class="zForm-label">{{__("Category Name (Arabic)")}}
                                        <span class="text-red">*</span></label>
                                    <input type="text" class="form-control zForm-control" id="createCategoryNameInAr"
                                           placeholder="{{__('Category Name (Arabic)')}}" name="name_ar"/>
                                </div>
                                <div class="col-12">
                                    ...
                                    <select class="sf-select-two" name="status">
                                        <option value="{{JOURNAL_CATEGORY_STATUS_ACTIVE}}">{{__("Active")}}</option>
                                        <option value="{{JOURNAL_CATEGORY_STATUS_INACTIVE}}">{{__("Inactive")}}</option>
                                    </select>
                                </div>
                            </div>
                        </div>
