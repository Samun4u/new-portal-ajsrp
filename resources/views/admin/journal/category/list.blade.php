...
@section('content')
    <span id="searchresult">
            @if(count($journalCategoryList) > 0)
            ...
            <div
                ...
                <div
                    ...
                    <!-- Add Service -->
                    <a href="{{route('admin.journals.category.add-new')}}"
                       class="border-0 bg-main-color py-8 px-26 bd-ra-8 fs-15 fw-600 lh-25 text-white">+ {{__("Add Category")}}</a>
                    <!-- Search -->
            ...
            <div class="p-sm-30 p-15">
                <div class="tab-content" id="myTabContent">
                    @if($pageType != 0 && $pageType == 2)
                        <div class="tab-pane fade " id="serviceGrid-tab-pane" role="tabpanel"
                                    ...
                                    <div class="col-xxl-3 col-md-4 col-sm-6">
                                        <div class="p-16 bd-ra-10 bg-white">
                                            ...
                                            <div class="bd-b-one bd-c-stroke pb-14 mb-11">
                                                <!--  -->
                                                <h4 class="fs-15 fw-600 lh-18 text-title-black pb-9">
                                                     @if (currentLang() == 'ar') {{ $category->arabic_name ? $category->arabic_name : $category->name }} @else {{ $category->name }} @endif
                                                </h4>
                                                <!--  -->
                                                <h4 class="fs-15 fw-500 lh-18 text-para-text">{{$service->status}}</h4>
                                            </div>
                                            <!-- Details -->
                                            <a href="{{route('admin.journals.category.edit', encrypt($$category->id))}}"
                            ...
                            <div class="row rg-15">
                                <div class="col-md-12 py-30">
                                    {{ $journalCategoryList->appends(['type' => 1])->links('admin.layouts.partial.common_pagination')}}
                                </div>
                            </div>
                                ...
                                <tbody>
                                    @foreach($journalCategoryList as $category)
                                        <tr>
                                            ...
                                            <td><p class='text-nowrap fs-12 fw-500 lh-15 text-para-text'>
                                                @if (currentLang() == 'ar') {{ $category->arabic_name ? $category->arabic_name : $category->name }} @else {{ $category->name }} @endif
                                            </p></td>
                                            <td><p class='fs-12 fw-500 lh-15 text-para-text'>{{$category->status}}</p></td>
                                            <td><a href='{{route('admin.journals.category.edit', encrypt($category->id))}}'
                        ...
                        <div class="row rg-15">
                            <div class="col-md-12 py-30">
                                {{ $journalCategoryList->appends(['type' => 2])->links('admin.layouts.partial.common_pagination')}}
                            </div>
                        </div>
                    </div>
                    @else
                                    ...
                                    <div class="col-xxl-3 col-md-4 col-sm-6">
                                        <div class="p-16 bd-ra-10 bg-white">
                                            ...
                                            <div class="bd-b-one bd-c-stroke pb-14 mb-11">
                                                ...
                                                <!--  -->
                                                <h4 class="fs-15 fw-500 lh-18 text-para-text">{{$category->status}}</h4>
                                            </div>
                                            <!-- Details -->
                                            <a href="{{route('admin.journals.category.edit', encrypt($category->id))}}"
                                               class="d-flex justify-content-between align-items-center cg-8 fs-14 fw-500 lh-17 text-main-color">
        ...
        @else
            ...
            <div class="p-sm-30 p-15 bg-white bd-one bd-c-stroke bd-ra-10">
                <div class="create-wrap">
                    ...
                    <h4 class="pb-22 fs-24 fw-500 lh-30 text-title-black text-center">{{__("There is no category available here!")}}</h4>
                    <a href="{{route('admin.journals.category.add-new')}}"
                       class="d-inline-flex bd-ra-8 bg-main-color py-10 px-26 fs-15 fw-600 lh-25 text-white">+{{__("Add Category")}}</a>
                </div>
            </div>
        </div>
        @endif
    ...
    <input type="hidden" id="serviceSearchRoute" value="{{route('admin.journals.category.search')}}">
@endsection
