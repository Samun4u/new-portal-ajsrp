...
@section('content')
    <span id="searchresult">
            ...
            <div
                ...
                <div
                    ...
                    <!-- Add Service -->
                    <a href="{{route('admin.journals.add-new')}}"
                       class="border-0 bg-main-color py-8 px-26 bd-ra-8 fs-15 fw-600 lh-25 text-white">+ {{__("Add Journal")}}</a>
                    <!-- Search -->
            ...
            <div class="p-sm-30 p-15">
                <div class="tab-content" id="myTabContent">
                    @if($pageType != 0 && $pageType == 2)
                        <div class="tab-pane fade " id="serviceGrid-tab-pane" role="tabpanel"
                                    ...
                                    <div class="col-xxl-3 col-md-4 col-sm-6">
                                        <div class="p-16 bd-ra-10 bg-white">
                                            ...
                                            <div class="bd-b-one bd-c-stroke pb-14 mb-11">
                                                <!--  -->
                                                <h4 class="fs-15 fw-600 lh-18 text-title-black pb-9">@if (currentLang() == 'ar') {{ $journal->arabic_title ? $journal->arabic_title : $journal->title }} @else {{ $journal->title }} @endif</h4>
                                                <!--  -->
                                                <h4 class="fs-15 fw-500 lh-18 text-para-text"><a href="{{$journal->website}}" target="_blank">{{__('Website')}}</a></h4>
                                                 <h4 class="fs-15 fw-500 lh-18 text-para-text">
                                                    @if($journal->service)
                                                    {{currentCurrency('symbol')}}{{$journal->service->price}}
                                                    @endif
                                                </h4> 
                                                <h4 class="fs-15 fw-500 lh-18 text-para-text">
                                                    @if (currentLang() == 'ar')
                                                        {{ $journal->subjects->map(fn($c) => $c->arabic_name ?? $c->name)->join(', ') }}
                                                    @else
                                                        {{ $journal->subjects->map(fn($c) => $c->name)->join(', ') }}
                                                    @endif
                                                </h4>
                                                <h4 class="fs-15 fw-500 lh-18 text-para-text">{{$journal->status}}</h4>
                                            </div>
                                            <!-- Details -->
                                            <a href="{{route('admin.journals.edit', encrypt($journal->id))}}"
                            ...
                            <div class="row rg-15">
                                <div class="col-md-12 py-30">
                                    {{ $journalList->appends(['type' => 1])->links('admin.layouts.partial.common_pagination')}}
                                </div>
                            </div>
                        ...
                        <div class="bg-white bd-one bd-c-stroke bd-ra-10 p-sm-30">
                            <table class="table zTable zTable-last-item-right" id="serviceListTable">
                                ...
                                <tr>
                                    ...
                                    <th>
                                        <div class="text-nowrap">{{__("Journal Title")}}</div>
                                    </th>
                                    <th>
                                        <div>{{__("Website")}}</div>
                                    </th>
                                ...
                                <tbody>
                                    @foreach($journalList as $journal)
                                        <tr>
                                            ...
                                            <td>
                                                <p class='text-nowrap fs-12 fw-500 lh-15 text-para-text'>@if (currentLang() == 'ar') {{ $journal->arabic_title ? $journal->arabic_title : $journal->title }} @else {{ $journal->title }} @endif</p>
                                            </td>
                                            <td><p class='fs-12 fw-500 lh-15 text-para-text'><a href="{{$journal->website}}" target="_blank">{{__("Website")}}</a></p></td>
                                             <td><p class='fs-12 fw-500 lh-15 text-para-text'>
                                                @if($journal->service)
                                                {{currentCurrency('symbol')}}{{$journal->service->price}}
                                                @endif
                                            </p></td> 
                                            <td>
                                                <p class='fs-12 fw-500 lh-15 text-para-text'>
                                                    @if (currentLang() == 'ar')
                                                        {{ $journal->subjects->map(fn($c) => $c->arabic_name ?? $c->name)->join(', ') }}
                                                    @else
                                                        {{ $journal->subjects->map(fn($c) => $c->name)->join(', ') }}
                                            ...
                                            <td><p class='fs-12 fw-500 lh-15 text-para-text'>{{$journal->status}}</p></td>
                                            <td><a href='{{route('admin.journals.edit', encrypt($journal->id))}}'
                                                   class='d-flex justify-content-center align-items-center w-30 h-30 rounded-circle bd-one bd-c-stroke ms-auto' style="background-color: var(--main-color) !important;"><img
                                                        src='{{ asset('assets/images/icon/edit.svg') }}' alt=''></a></td>
                                        </tr>
                                    @endforeach
                                </tbody>
                            </table>
                        ...
                        <div class="row rg-15">
                            <div class="col-md-12 py-30">
                                {{ $journalList->appends(['type' => 2])->links('admin.layouts.partial.common_pagination')}}
                            </div>
                        </div>
                    </div>
                    @else
                                ...
                                <table class="table zTable zTable-last-item-right" id="serviceListTable">
                                    ...
                                    @foreach($journalList as $journal)
                                        <tr>
                                            ...
                                            <td>
                                                <p class='text-nowrap fs-12 fw-500 lh-15 text-para-text'>
                                                    @if (currentLang() == 'ar') {{ $journal->arabic_title ? $journal->arabic_title : $journal->title }} @else {{ $journal->title }} @endif
                                                </p>
                                            </td>
                                            <td><p class='fs-12 fw-500 lh-15 text-para-text'><a href="{{$journal->website}}" target="_blank">{{ __("Website") }}</a></p></td>
                                             <td><p class='fs-12 fw-500 lh-15 text-para-text'>
                                            ...
                                            <td><p class='fs-12 fw-500 lh-15 text-para-text'>{{$journal->status}}</p></td>
                                            <td><a href='{{route('admin.journals.edit', encrypt($journal->id))}}'
                                                   class='d-flex justify-content-center align-items-center w-30 h-30 rounded-circle bd-one bd-c-stroke ms-auto rtl-button' style="background-color: var(--main-color) !important;"><img
                                                        src='{{ asset('assets/images/icon/edit.svg') }}' alt=''></a>
                                            </td>
                                        </tr>
                                    @endforeach
                                    </tbody>
        ...
        @else
            ...
            <div class="p-sm-30 p-15 bg-white bd-one bd-c-stroke bd-ra-10">
                <div class="create-wrap">
                    ...
                    <h4 class="pb-22 fs-24 fw-500 lh-30 text-title-black text-center">{{__("There is no Journal available here!")}}</h4>
                    <a href="{{route('admin.journals.add-new')}}"
                       class="d-inline-flex bd-ra-8 bg-main-color py-10 px-26 fs-15 fw-600 lh-25 text-white">+{{__("Add Journal")}}</a>
                </div>
            </div>
        </div>
        @endif
    ...
    <input type="hidden" id="serviceSearchRoute" value="{{route('admin.journals.search')}}">
@endsection
