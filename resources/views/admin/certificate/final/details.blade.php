...
<body>
    ...
    <div class="container">
        ...
        <div class="meta-info">
            Date: {{ $date ?? \Carbon\Carbon::now()->format('d/m/Y') }}<br>
            Ref: AJSRP/ACC/{{ $ref_no ?? $order_id ?? 'N/A' }}
        </div>
        ...
        <div class="content">
            ...
            @else
                This certificate is proudly presented to <strong>{{ $author ?? '' }}</strong> from <strong>{{ $affiliation ?? '' }}</strong> for the final acceptance of the scientific research titled:
            @endif
        </div>
        ...
        <div class="manuscript-details">
            <table>
                ...
                <tr>
                    <td class="label">@if($isArabic) المجلد / العدد @else Volume / Issue: @endif</td>
                    <td><strong>{{ $volume ?? 'N/A' }} / {{ $issue ?? 'N/A' }}</strong></td>
                </tr>
            </table>
        </div>
        ...
        <div class="content">
            @if($isArabic)
                {{ $journalInfoParaOneStatic ?? '' }} <strong>{{ $journal_name ?? '' }}</strong>، {{ $journalInfoParaFourStatic ?? '' }} {{ $date ?? '' }}.
            @else
                The manuscript has been accepted for publication in <strong>{{ $journal_name ?? '' }}</strong> and is scheduled for release.
            @endif
        </div>
