...
@push('title')
    {{ __('Galley (Final Layout) Management') }}
@endpush
...
@section('content')
    <div class="galley-container">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h2>{{ __('Galley (Final Layout) Management') }}</h2>
        </div>

        <!-- Filters -->
        <div class="filter-section">
            <form method="GET" action="{{ route('admin.galley.index') }}">
                ...
                <div class="d-flex gap-2">
                    ...
                    <a href="{{ route('admin.galley.index') }}"
            ...
            @foreach($submissions as $submission)
                <div class="submission-card">
                    <div class="submission-header">
                            ...
                            <div class="submission-meta">
                                ...
                                @if($submission->galleyFiles && $submission->galleyFiles->count() > 0)
                                    <div class="meta-item">
                                        ...
                                        <span>{{ $submission->galleyFiles->count() }} {{ __('Galley Version(s)') }}</span>
                                    </div>
                                @endif
                            </div>
                            @if($submission->authors && $submission->authors->count() > 0)
                        ...
                        <div class="galley-status">
                            @if($submission->galleyFiles && $submission->galleyFiles->count() > 0)
                                @php
                                    $hasApproved = $submission->galleyFiles->contains('status', 'approved');
                                    $latestGalley = $submission->galleyFiles->sortByDesc('created_at')->first();
                                @endphp
                                @if($hasApproved)
                                    <span class="badge-galley badge-approved">
                                        <i class="fa fa-check-circle"></i> {{ __('Galley Approved') }}
                                    </span>
                                @else
                                    <span class="badge-galley badge-pending">
                                        <i class="fa fa-clock"></i> {{ __('Pending Review') }}
                                    </span>
                                @endif
                            @else
                                <span class="badge-galley badge-pending">
                                    <i class="fa fa-exclamation-circle"></i> {{ __('No Galley Uploaded') }}
                                </span>
                            @endif
                                   <div class="d-flex gap-2 align-items-center">
                                       <a href="{{ route('admin.galley.list', encrypt($submission->id)) }}"
                                          class="btn-manage">
                                           <i class="fa fa-cog"></i> {{ __('Manage') }}
                                       </a>
                                       @if($submission->metadata_status === 'approved' && $hasApproved)
                                           <a href="{{ route('admin.ojs.quicksubmit-data', encrypt($submission->id)) }}"
                                              class="btn-manage" style="background: #28a745;">
        ...
        @else
            <div class="alert alert-info">
                <i class="fa fa-info-circle"></i> {{ __('No submissions in galley stage found.') }}
            </div>
        @endif
    </div>
@endsection
